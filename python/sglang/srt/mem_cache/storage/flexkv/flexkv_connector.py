...
class FlexKVConnector(BaseKVConnector):
        ...
        if self._sync_ctx.is_sync_leader:
            ...
            if use_swa_match:
                try:
                    ...
                    logger.warning(
                        "[FlexKV] get_match_swa failed (%s: %s); falling back to get_match",
                        type(swa_err).__name__, swa_err,
                        exc_info=True,
                    )
    ...
    def start_load_kv(
        ...
        # SWA-pool slot_mappings, aligned 1:1 with flexkv_task_ids. Each entry is
        # the trailing SWA window (or None when SWA is off / no window), used to
        # late-bind the SWA H2D op's GPU dst via launch(swa_slot_mappings=). None
        # entries leave the SWA op untouched (FlexKV builds an SWA op only when it
        # found an SWA hit, and a real GET load always carries the tail window, so
        # a None here should not coincide with a built SWA op — logged if it does).
        swa_slot_mappings: List[Optional[np.ndarray]] = []
        ...
        for op in load_ops:
            ...
            swa_slot_mappings.append(self._build_swa_slot_mapping(indices))
            try:
        ...
        if self.enable_layerwise_transfer and self._layer_done_counter is not None:
            ...
            if self._sync_ctx.is_sync_leader:
                ...
                self.kv_manager.launch(
                    ...
                    swa_slot_mappings=self._swa_launch_arg(swa_slot_mappings),
                )
    ...
    def start_store_kv(
        ...
        try:
            ...
            if unmatched_mask.sum() > 0:
                ...
                # SWA D2H stores the trailing window of the FULL stored sequence
                # (the stored tail node's own window), not the unmatched subset:
                # FlexKV's _swa_put_slots mounts one fresh SWA slot on that tail
                # node. Build it from the page-aligned kv_indices tail; None (SWA
                # off / <1 window) leaves the store as full-KV-only.
                swa_sm = self._build_swa_slot_mapping(kv_indices)
                self.kv_manager.launch(
                    task_ids=[fkv_task_id], slot_mappings=[slot_mapping],
                    swa_slot_mappings=self._swa_launch_arg([swa_sm]),
                )
                self._ongoing_stores[task_id] = fkv_task_id
            else:
                self._completed_stores.append(task_id)
    ...
    def _register_to_server_dsv4(
        ...
        swa_caches, swa_layout = self._build_swa_registration()
        self.tp_client.register_to_server(
        ...
        logger.info(
            ...
            + (f" (+ SWA GPU pool, {len(swa_caches)} layers)"
               if swa_caches else " (no SWA GPU pool)")
        )

    def _build_swa_registration(self):
        """Build the SWA dedicated GPU pool registration payload (channel B).

        Returns ``(swa_caches, swa_layout)`` for
        ``KVTPClient.register_to_server(swa_caches=, swa_layout=)`` — the CUDA-IPC
        handles for the sliding-window GPU buffers so the FlexKV worker can map
        them independently of the main-KV pool and move SWA bytes on the data
        plane. Returns ``(None, None)`` (SWA stays control-plane-only / CPU
        resident) when there is no SWA GPU pool or the data plane is disabled,
        so a missing/odd SWA pool never blocks main-KV registration.

        The DSv4 SWA sub-pool (``DeepSeekV4SingleKVPool``) stores KV in a list of
        2D uint8 buffers ``[num_pages, bytes_per_page_padded]`` — the SAME packed
        layout as the c4/c128 sub-pools, but its own ``swa_page_size`` (256) and a
        single per-token width including 576-byte page-alignment padding. The
        registered ``head_size`` is that PADDED per-token width (149760/256 = 585,
        NOT the logical 584), which must equal the FlexKV host SWA pool's
        ``bytes_per_token_per_layer`` (set to 585 in integration/config.py) so the
        H2D/D2H byte offsets line up with the GPU buffer stride.
        """
        pool = self._swa_kv_pool
        if pool is None:
            return None, None
        # Gate on the data plane being enabled (mirrors the FlexKV-side
        # enable_swa_transfer gate; when off, the transfer graph builds no SWA
        # ops so a registered GPU pool would just sit unused).
        cache_config = self.flexkv_config.cache_config
        swa_cfg = getattr(cache_config, "swa", None)
        if swa_cfg is None or not swa_cfg.enabled:
            logger.info(
                "[FlexKV-SWA] cache_config.swa disabled; skipping SWA GPU pool "
                "registration (SWA stays CPU-resident / control-plane only)"
            )
            return None, None
        ...
        if not buffers:
            logger.warning(
                "[FlexKV-SWA] SWA pool has no kv_buffer; skipping SWA GPU "
                "registration"
            )
            return None, None
        ...
        if buf0.ndim != 2:
            logger.warning(
                "[FlexKV-SWA] SWA buffer expected 2D [num_pages, "
                "bytes_per_page_padded], got shape=%s; skipping SWA GPU "
                "registration",
                tuple(buf0.shape),
            )
            return None, None
        if not buf0.is_cuda:
            logger.warning(
                "[FlexKV-SWA] SWA buffer is not CUDA; skipping SWA GPU "
                "registration"
            )
            return None, None

        num_pages = buf0.shape[0]
        bytes_per_page_padded = buf0.shape[1]
        # NOTE: _register_to_server_dsv4 runs BEFORE the __init__ block that sets
        # self._swa_window_size, so read the pool's own page_size (always present
        # on DeepSeekV4SingleKVPool = swa_page_size) and only fall back to a
        # getattr default — never to a not-yet-assigned attribute.
        swa_page_size = (
            getattr(pool, "page_size", None)
            or getattr(self, "_swa_window_size", 0)
        )
        if not swa_page_size or bytes_per_page_padded % swa_page_size != 0:
            logger.warning(
                "[FlexKV-SWA] SWA page bytes %d not divisible by swa_page_size "
                "%s; skipping SWA GPU registration (byte stride would shear)",
                bytes_per_page_padded, swa_page_size,
            )
            return None, None
        effective_head_size = bytes_per_page_padded // swa_page_size

        # Consistency check against the FlexKV host pool width so a mismatch
        # fails loudly here instead of silently corrupting SWA KV.
        host_bpt = getattr(swa_cfg, "bytes_per_token_per_layer", None)
        if host_bpt is not None and host_bpt != effective_head_size:
            logger.error(
                "[FlexKV-SWA] SWA GPU head_size=%d != host bytes_per_token=%d; "
                "byte offsets would not line up. Skipping SWA GPU registration "
                "to avoid corruption — fix SWAPoolConfig.bytes_per_token_per_layer.",
                effective_head_size, host_bpt,
            )
            return None, None

        # All buffers must share the shape we registered against.
        for li, b in enumerate(buffers):
            if b.shape != buf0.shape:
                logger.warning(
                    "[FlexKV-SWA] SWA buffer layer %d shape %s != %s; skipping "
                    "SWA GPU registration", li, tuple(b.shape), tuple(buf0.shape),
                )
                return None, None

        swa_layout = KVCacheLayout(
            type=KVCacheLayoutType.LAYERFIRST,
            num_layer=len(buffers),
            num_block=num_pages,
        ...
        logger.info(
            "[FlexKV-SWA] Registering SWA GPU pool: num_layers=%d, num_pages=%d, "
            "swa_page_size=%d, head_size=%d (padded), bytes_per_page=%d",
            len(buffers), num_pages, swa_page_size, effective_head_size,
            bytes_per_page_padded,
        )
        return list(buffers), swa_layout

    def _build_swa_slot_mapping(self, full_gpu_indices) -> Optional[np.ndarray]:
        """Translate full-pool GPU token indices to the SWA-pool token-index
        slot_mapping FlexKV's ``launch(swa_slot_mappings=)`` expects, for the
        TRAILING SWA window only. Returns ``None`` (no SWA byte movement for this
        task; degrade cleanly to full-KV) when SWA is disabled, no mapping is
        registered, or there is less than one window of tokens.

        Why only the trailing window: DSv4's SWA GPU pool is sized to ~one window
        per request and ``init_load_back`` allocates SWA slots for just the tail
        (``alloc_extend_swa_tail``); FlexKV likewise stores/loads exactly one
        window (one slot) per node. ``full_to_swa_index_mapping`` is only valid
        for tokens that actually have an SWA slot — the tail — so we take the last
        ``window`` full indices and translate those. FlexKV's
        ``swa_slot_mapping_to_slot_ids`` then folds this per-window
        (``sm[::window] // window``) into SWA-pool slot ids, matching the single
        trailing block that ``get_match_swa`` reports in ``return_mask_swa``.
        """
        if self._swa_kv_pool is None:
            return None
        window = self._swa_window_size or 0
        if window <= 0:
            return None
        kvcache = self._kvcache
        if kvcache is None or not hasattr(kvcache, "translate_loc_from_full_to_swa"):
            return None
        if getattr(kvcache, "full_to_swa_index_mapping", None) is None:
            return None

        try:
            idx = full_gpu_indices
            if not torch.is_tensor(idx):
                idx = torch.as_tensor(idx)
            idx = idx.flatten()
            n = int(idx.numel())
            # Need at least one full window to form a single SWA slot; the tail
            # must also be window-aligned so [::window] lands on window starts.
            if n < window:
                return None
            aligned = (n // window) * window
            # Trailing window: the last `window` tokens of the page-aligned span.
            tail = idx[aligned - window: aligned]
            if int(tail.numel()) != window:
                return None
            tail = tail.to(dtype=torch.int64)
            swa_idx = kvcache.translate_loc_from_full_to_swa(tail)
            swa_idx_cpu = swa_idx.cpu() if swa_idx.is_cuda else swa_idx
            swa_np = swa_idx_cpu.to(torch.int64).numpy()
            # Guard against the -1 sentinel (unmapped) leaking in: a negative slot
            # id would index the SWA pool out of bounds. If any tail token has no
            # SWA slot, skip SWA movement for this task rather than corrupt.
            if swa_np.size == 0 or (swa_np < 0).any():
                return None
            return swa_np
        except Exception as e:  # noqa: BLE001 — never crash the scheduler over SWA
            logger.warning(
                "[FlexKV-SWA] failed to build swa_slot_mapping (%s); "
                "degrading to full-KV-only for this task", e,
            )
            return None
    ...
    def _swa_launch_arg(
        ...
        """Collapse a list of per-task SWA slot_mappings to the ``launch``
        argument: ``None`` when every entry is ``None`` (no SWA binding at all, so
        FlexKV skips the SWA late-bind branch entirely), otherwise the list as-is
        (kept 1:1 with task_ids; ``None`` entries leave that task's SWA op — if
        any — unbound). This keeps the common no-SWA path identical to before."""
        if not swa_slot_mappings or all(sm is None for sm in swa_slot_mappings):
            return None
        return swa_slot_mappings
